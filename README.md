...
The child process was started with command line arguments _:to-stderr :write-nl 2_, its PID was _36744_ and its standard error was attached to the console (_'*'_) since it has inherited from its parent process (the default).
...
**Note**: The tool reports with _*_ in its commentary that stderr is attached to the console.
...
Specifying a pipe-size of 0, create a pipe with a size of 4096 bytes long (the default size)
...
When spawning a child and redirecting its stderr to a _pipe's write endpoint, the child's stderr stream mode becomes fully buffered:
...
Notice in the above that, in the first case (writing 4095 bytes) the parent only received the output after the child has exited (and thus the stderr buffers were flush), while in the second case (writing 4096 bytes) the parent received the data immediately with the write, before the child exited.
...
and as expected, trying to write more bytes to stderr than there is available space on the pipe buffer will causes the write operation to block
...
The write socket has to be of a special type that can be treated as a file handle. It appears that they could be many types of such sockets registered on a system, the tool will select the first available. 

The below tests were conducted using a write socket type of _MSAFD Tcpip [TCP/IP]_ (the first one reported with [WSAEnumProtocols](https://docs.microsoft.com/en-us/windows/win32/api/winsock2/nf-winsock2-wsaenumprotocolsa) fn).
...
The Emacs shell (can be invoked with _M-x shell_), invokes the _cmdproxy.exe_ utility.
